...
The same boundary applies to MCP: `search_context` in lexical mode and `index_status` are provider-inert. Semantic/hybrid search, or auto when it selects hybrid, may contact only the explicitly enabled endpoint. `omnisem mcp` adds no configuration fields and cannot mutate configuration.
